...
public class ProductFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        // 原设计经 productPermissionConfig 解析 URL→权限组，再查 user_product 有效期做拦截（抛 PRODUCT_NOT_AUTHORIZED=5001）。
        // 此处首行直接放行 + return，作为去付费双保险的第二道闸门（与 UserProductMapper 恒返回互补），
        // 也兜底 SmartConfig 动态下发的 product.url.map 新增 URL。
